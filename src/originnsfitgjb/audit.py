...
STEP_PURPOSES = {
    ...
    "Step06_ParameterSignificance": "Check A4 action rule first and record A2 life-strain significance as a note.",
    "Step07_FixedA4LinearFit": "Fix A4 and re-estimate A1/A2 linearly when A4 lower CI is negative or weighted correction is selected.",
...
STEP_FORMULAS = {
    ...
    "Step06_ParameterSignificance": "A4_lower_90 < 0 triggers A4=0 linear refit; A2_upper_90 < 0 is recorded only as the life-strain significance note",
    "Step07_FixedA4LinearFit": "With fixed A4, X=log10(response-A4); weighted: Y*=y/h, U=1/h, V=X/h; Y*=A1_corrected*U + A2_corrected*V",
...
STEP_INPUTS = {
    ...
    "Step07_FixedA4LinearFit": "Step05 refit rows, selected fixed A4, h and weight",
...
STEP_RULES = {
    ...
    "Step06_ParameterSignificance": "A4 lower 90% < 0 fixes A4=0 later; A2 significance never blocks the workflow",
    "Step07_FixedA4LinearFit": "perform when A4 lower 90% < 0, otherwise perform only when weighted refit was selected",
    "Step08_ResidualsOutliers": "auto removes candidate only when G > critical; report-only never removes",
    "Step09_FinalMLE": "run after Step06 records the A4 action rule; domain-valid runout rows contribute as right-censored logsf terms",
...
def _manual_check_items() -> list[dict[str, str]]:
    return [
        ...
        _check("13", "Step06_ParamSignif", "if A4_lower_90 < 0, subsequent A4 is fixed at 0", "lower_90,decision", ""),
        _check("14", "Step06_ParamSignif", "A2 significance is a note only and does not stop later steps", "passed,decision,affects_workflow", ""),
        _check("15", "Step07_FixedA4Linear", "only A1/A2 corrected; A4 fixed", "A4_fixed", ""),
        _check("16", "Step07_FixedA4Linear", "A1^2 and A2^2 are corrected params, not squares", "A1_corrected,A2_corrected", ""),
        _check("17", "Step10_FinalResiduals", "three-parameter model k=3", "k", ""),
        _check("18", "Step09_FinalMLE", "domain-valid runout likelihood uses logsf", "likelihood_type", ""),
        _check("19", "Origin workbook", "Origin Direct Weighting receives 1/h^2, not h", "weight", ""),
        _check("20", "Step09_FinalMLE", "rows with response <= A4 are ignored in MLE", "domain_valid,included_in_final_mle,A4_final_mle", ""),
    ]
